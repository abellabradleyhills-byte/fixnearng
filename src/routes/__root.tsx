  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="max-w-md text-center">
        <h1 className="text-7xl font-display font-bold text-brand-green">404</h1>
        <h2 className="mt-4 text-xl font-semibold">Page not found</h2>
        <p className="mt-2 text-sm text-muted-foreground">
          This page doesn't exist. Head back home to find an artisan.
        </p>
        <div className="mt-6">
          <Link
            to="/"
            className="inline-flex items-center justify-center rounded-xl bg-brand-green px-4 py-2 text-sm font-semibold text-white"
          >
            Back to FixNear
          </Link>
        </div>
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "FixNear — Find verified artisans near you" },
      {
        ...
        content:
          "NIN & BVN verified skilled artisans across Lagos, Abuja, Lokoja and beyond. Emergency roadside SOS built-in.",
      },
...
function RootComponent() {
  ...
  useEffect(() => {
    import("../lib/prefs").then(({ applyTheme }) => applyTheme());
  }, []);
  return (
